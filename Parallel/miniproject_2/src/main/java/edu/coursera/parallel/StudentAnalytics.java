...
public final class StudentAnalytics {
    ...
    public String mostCommonFirstNameOfInactiveStudentsParallelStream(
            final Student[] studentArray) {
        Map<String,Long> res = Stream.of(studentArray)
                .parallel()
                .filter(f->!f.checkIsCurrent())
                .map(m->m.getFirstName())
                .collect(Collectors.groupingBy(k->k,Collectors.counting()));
        return res.entrySet().stream().parallel().max(Comparator.comparing(Map.Entry::getValue)).get().getKey();
    }
    ...
    public int countNumberOfFailedStudentsOlderThan20ParallelStream(
            final Student[] studentArray) {
        long res = Stream.of(studentArray).parallel()
                .filter(f->!f.checkIsCurrent()&&f.getAge()>20&&f.getGrade()<65)
                .count();
        return (int) res;
    }
}
